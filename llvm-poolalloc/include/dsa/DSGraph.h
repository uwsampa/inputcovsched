...
private:
  ...
  // Thread creations - A list of dummy callsites representing the thread creation
  // operations performed by functions merged into this graph.  For example, for
  // pthread_create(&th, NULL, run, arg), we generate a dummy callsite `run(arg)`.
  // CS.getInstruction() points to the original pthread_create call.
  std::list<DSCallSite> CreateThreadCalls;

  // Erased calls - For users that need a complete callgraph, this is the set of
  // call that are removed from the callgraph for some reason (e.g., StdLibPass
  // removes known standard library calls).
  std::list<DSCallSite> RemovedFunctionCalls;

  // A set of pointers that are only ever used as part of a memset() operation.
  // If a value exists in this set, then (a) that value is used by exactly one
  // StoreInst that stores a constant value (e.g., 0) of type i8, and (b) that
  // value should be assumed to alias the entire DSNode, not just the offset
  // specified by the DSNodeHandle.  See UGLY_GEP_HACK in Local.cpp
  std::set<const Value*> MemsetHackSet;
...
public:
  ...
  /// getCreateThreadCalls - Get the list of dummy thread creation callsites.
  ///
  std::list<DSCallSite> &getCreateThreadCalls() { return CreateThreadCalls; }
  const std::list<DSCallSite> &getCreateThreadCalls() const {
    return CreateThreadCalls;
  }

  /// getRemovedFunctionCalls - Get the list of removed callsites.
  ///
  std::list<DSCallSite> &getRemovedFunctionCalls() { return RemovedFunctionCalls; }
  const std::list<DSCallSite> &getRemovedFunctionCalls() const {
    return RemovedFunctionCalls;
  }
  ...
  /// ugly hack - see above
  bool isPointerUsedForMemsetOnly(const llvm::Value *v) const {
    return MemsetHackSet.count(v) > 0;
  }
  void addPointerUsedForMemsetOnly(const llvm::Value *v) {
    MemsetHackSet.insert(v);
  }
