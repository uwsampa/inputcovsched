...
BUDataStructures::calculateGraphs (const Function *F,
  ...
  if (Stack.back() == F) {           // Special case the single "SCC" case here.
    ...
    if (!CalleeFunctions.empty()) {
      ...
      // N.B.: The DSA paper says to erase ValMap[f] forall f in SCC
      // This is the only way I know of enumerating all f merged into the graph
      bool found = false;
      for (DSGraph::retnodes_iterator
           I = SCCGraph->retnodes_begin(); I != SCCGraph->retnodes_end(); ++I) {
        ValMap.erase(I->first);
        if (I->first == F)
          found = true;
      }
      assert(found);
      ...
      // N.B.: We probably need to do this forall functions in the graph, as above
      bool found = false;
      for (DSGraph::retnodes_iterator
           I = SCCGraph->retnodes_begin(); I != SCCGraph->retnodes_end(); ++I) {
        ValMap[I->first] = ~0U;
...
void BUDataStructures::calculateGraph(DSGraph* Graph) {
  ...
  while (!TempFCs.empty()) {
    ...
    for (unsigned x = 0; x < CalledFuncs.size(); ++x) {
      ...
      // XXX: ugly
      if (SteensgaardDataStructures::isIgnoredFunction(Callee))
        continue;
