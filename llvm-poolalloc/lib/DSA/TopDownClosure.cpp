...
void TDDataStructures::ComputePostOrder(const Function &F,
  ...
  // Also traverse dummy callsites for thread create operations
  for (std::list<DSCallSite>::const_iterator CS = G->getCreateThreadCalls().begin();
                                             CS != G->getCreateThreadCalls().end(); ++CS) {
    std::vector<const Function*> Targets;
    CS->getCalleeNode()->addFullFunctionList(Targets);
    for (size_t i = 0; i < Targets.size(); ++i)
      ComputePostOrder(*Targets[i], Visited, PostOrder);
  }
...
void TDDataStructures::InlineCallersIntoGraph(DSGraph* DSG) {
  ...
  while (!EdgesFromCaller.empty()) {
    ...
    do {
      ...
      // XXX: ugly condition
      if (!SteensgaardDataStructures::isIgnoredFunction(&CF)) {
  ...
  // Loop over all the thread creations and for each, create a dummy caller edge
  // to all possible thread functions.
  // TODO: incomplete/external nodes?
  for (std::list<DSCallSite>::const_iterator CS = DSG->getCreateThreadCalls().begin();
                                             CS != DSG->getCreateThreadCalls().end(); ++CS) {
    DEBUG(errs() << "[TD] Dummy thread callsite in "
                 << DSG->getFunctionNames() << "\n");
    std::vector<const Function*> Targets;
    CS->getCalleeNode()->addFullFunctionList(Targets);
    for (size_t i = 0; i < Targets.size(); ++i) {
      const Function *T = Targets[i];
      // Skip over declarations and calls to this graph (same-SCC calls)
      if (T->isDeclaration() || getDSGraph(*T) == DSG)
        continue;
      CallerEdges[getDSGraph(*T)].push_back(CallerCallEdge(DSG, &*CS, T));
      DEBUG(errs() << "[TD] Dummy thread callsite calls " << T->getNameStr() << "\n");
    }
  }
}
