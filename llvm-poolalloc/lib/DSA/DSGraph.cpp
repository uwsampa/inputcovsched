...
DSGraph::~DSGraph() {
  ...
  CreateThreadCalls.clear();
  RemovedFunctionCalls.clear();
  ScalarMap.clear();
  ReturnNodes.clear();
  VANodes.clear();
...
void DSGraph::removeFunctionCalls(Function& F) {
  // Two notes about ugliness:
  // * This function is invoked once-per-callsite (see StdLibDataStructures::eraseCallsTo),
  //   so the "break"s below end up not mattering.  This could be much more efficient, but
  //   it doesn't matter in practice.
  // * At the time this function is invoked, FunctionCalls == AuxFunctionCalls, thus, we
  //   are careful to push callsites onto RemovedFunctionCalls just once.

  for (std::list<DSCallSite>::iterator I = FunctionCalls.begin(),
         E = FunctionCalls.end(); I != E; ++I)
    if (I->isDirectCall() && I->getCalleeFunc() == &F) {
      RemovedFunctionCalls.push_back(*I);
...
void DSGraph::cloneInto(DSGraph* G, unsigned CloneFlags) {
  ...
  if (!(CloneFlags & DontCloneCallNodes)) {
    // Copy the thread creations list.
    for (fc_iterator I = G->getCreateThreadCalls().begin(),
                     E = G->getCreateThreadCalls().end(); I != E; ++I)
      CreateThreadCalls.push_back(DSCallSite(*I, OldNodeMap));
    // Copy the removed functions list
    for (fc_iterator I = G->getRemovedFunctionCalls().begin(),
                     E = G->getRemovedFunctionCalls().end(); I != E; ++I)
      RemovedFunctionCalls.push_back(DSCallSite(*I, OldNodeMap));
  }
  ...
  // Copy
  MemsetHackSet.insert(G->MemsetHackSet.begin(), G->MemsetHackSet.end());
}
...
void DSGraph::spliceFrom(DSGraph* RHS) {
  ...
  CreateThreadCalls.splice(CreateThreadCalls.end(), RHS->CreateThreadCalls);
  RemovedFunctionCalls.splice(RemovedFunctionCalls.end(), RHS->RemovedFunctionCalls);
  ...
  // Ditto
  if (MemsetHackSet.empty()) {
    MemsetHackSet.swap(RHS->MemsetHackSet);
  } else {
    MemsetHackSet.insert(RHS->MemsetHackSet.begin(), RHS->MemsetHackSet.end());
    RHS->MemsetHackSet.clear();
  }
...
void DSGraph::mergeInGraph(const DSCallSite &CS,
  ...
  std::vector<const DSCallSite*> CreateCallToCopy;
  std::vector<const DSCallSite*> RemovedCallToCopy;
...
//        }
  ...
  if (!(CloneFlags & DontCloneAuxCallNodes)) {
    for (afc_const_iterator I = Graph.getCreateThreadCalls().begin(),
                            E = Graph.getCreateThreadCalls().end(); I!=E; ++I) {
      if (SCCFinder.PathExistsToClonedNode(*I))
        CreateCallToCopy.push_back(&*I);
    }
    for (afc_const_iterator I = Graph.getRemovedFunctionCalls().begin(),
                            E = Graph.getRemovedFunctionCalls().end(); I!=E; ++I) {
      if (SCCFinder.PathExistsToClonedNode(*I))
        RemovedCallToCopy.push_back(&*I);
    }
  }
  ...
  // Copy create calls that are needed.
  for (unsigned i = 0, e = CreateCallToCopy.size(); i != e; ++i)
    CreateThreadCalls.push_back(DSCallSite(*CreateCallToCopy[i], RC));

  // Copy removed calls that are needed.
  for (unsigned i = 0, e = RemovedCallToCopy.size(); i != e; ++i)
    RemovedFunctionCalls.push_back(DSCallSite(*RemovedCallToCopy[i], RC));
  ...
  // Probably don't need to always copy this, but meh, it can't hurt...
  MemsetHackSet.insert(Graph.MemsetHackSet.begin(), Graph.MemsetHackSet.end());
}
...
void DSGraph::removeDeadNodes(unsigned Flags) {
  ...
  // Mark dummy thread creation callsites alive
  for (std::list<DSCallSite>::const_iterator CS = CreateThreadCalls.begin();
                                             CS != CreateThreadCalls.end(); ++CS) {
    CS->markReachableNodes(Alive);
  }

  // Mark dummy removed callsites alive
  for (std::list<DSCallSite>::const_iterator CS = RemovedFunctionCalls.begin();
                                             CS != RemovedFunctionCalls.end(); ++CS) {
    CS->markReachableNodes(Alive);
  }
...
llvm::functionIsCallable (CallSite CS, const Function* F) {
  ...
  if (!noDSACallFP) {
    ...
    for (unsigned index = 0; index < NumParams; ++index) {
      const Type * AType = F->getFunctionType()->getParamType(index);
      const Type * PType = FT->getParamType(index);
