...
#ifdef UGLY_GEP_HACK
  static bool isAllocationInst(llvm::Value *v) {
    if (isa<AllocaInst>(v))
      return true;
    CallInst *call = dyn_cast<CallInst>(v);
    Function *callee = call ? call->getCalledFunction() : NULL;
    return callee && (callee->getName() == "malloc" || callee->getName() == "calloc");
  }
#endif
}
...
void GraphBuilder::visitStoreInst(StoreInst &SI) {
  const Type *StoredTy = SI.getValueOperand()->getType();
  DSNodeHandle Dest = getValueDest(SI.getPointerOperand());
  ...
  if (isa<PointerType>(StoredTy))
    Dest.addEdgeTo(getValueDest(SI.getValueOperand()));

  if(TypeInferenceOptimize)
    if(SI.getValueOperand()->getNumUses() == 1)
...
#ifdef UGLY_GEP_HACK
        gep_type_iterator NextI = I;
        if (!Value.getNode()->isArrayNode() && !Value.getOffset() && Offset == 0
            && ++NextI == E && TD.getTypeAllocSize(CurTy) == 1) {
          // Ugly special case #1 (can happen due to GVN):
          //   %raw = call i8* @malloc(...)
          //   %GEP = getelementptr %raw, <const>
          //   %foo = bitcast i8* %GEP to <elemtype>
          ConstantInt *stride = dyn_cast<ConstantInt>(I.getOperand());
          if (stride && GEP.hasOneUse() && isAllocationInst(GEP.getOperand(0))) {
            if (BitCastInst *bitcast = dyn_cast<BitCastInst>(*GEP.use_begin())) {
              if (bitcast->getDestTy()->isPointerTy()) {
                // Advance offset by <const>
                Offset += stride->getSExtValue();
                // Merge types
                const Type *ElemTy = cast<PointerType>(bitcast->getDestTy())->getElementType();
                Value.getNode()->mergeTypeInfo(ElemTy, Offset);
                continue;
              }
            }
          }
          // Ugly special case #2 (this is memset):
          //   %GEP = getelementptr %raw, <phi>
          //   %foo = store i8 <const>, i8* %GEP
          PHINode *phi = dyn_cast<PHINode>(I.getOperand());
          if (phi && GEP.hasOneUse()) {
            if (StoreInst *store = dyn_cast<StoreInst>(*GEP.use_begin())) {
              if (isa<ConstantInt>(store->getValueOperand())) {
                //errs() << "SETTING MEMSET FLAG"
                //       << "\n  .. gep: " << GEP
                //       << "\n  .. phi: " << *phi
                //       << "\n  .. store: " << *store
                //       << "\n";
                // N.B.: Users should assume that this value is a writes to the entire
                // node, *not* just to the byte at node[offset]
                G.addPointerUsedForMemsetOnly(&GEP);
                setDestTo(GEP, Value);
                return;
              }
            }
