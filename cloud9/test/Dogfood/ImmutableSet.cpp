...
// RUN: %klee --libc=uclibc --max-forks=200 --no-output --exit-on-error --optimize --disable-inlining --use-non-uniform-random-search --use-cex-cache %t1.bc
...
void check_set(T &set, unsigned num, unsigned *values) {
  ...
  unsigned item = klee_range32(0, 256, "range");
  ...
  unsigned item2 = klee_range32(0, 256, "range");
  if (!set.count(item2)) {
...
void test() {
  ...
  for (unsigned i=0; i<klee_range32(0,MAX_ELEMENTS+1, "range"); i++) {
    unsigned item = klee_range32(0, 256, "range");
