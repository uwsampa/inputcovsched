...
namespace klee {
  ...
  private:
    ...
    struct AlwaysMerge {
      typedef std::pair<bool, value_type> ResultTy;
      ResultTy operator()(const value_type &a, const value_type &b) const {
        return ResultTy(true, a);
      }
    };
  ...
  public:
    ...
    ImmutableSet popMin(value_type &valueOut) const {
      return elts.popMin(valueOut);
    }
    ImmutableSet popMax(value_type &valueOut) const {
      return elts.popMax(valueOut);
    }

    template<class Predicate>
    ImmutableSet remove_if(const Predicate &pred) const {
      return elts.remove_if(pred);
    }

    ImmutableSet union_with(const ImmutableSet &other) const {
      return elts.merge_with(other.elts, AlwaysMerge());
    }

    template<class MapFunc>
    ImmutableSet map_values(const MapFunc &mapfn) const {
    ...
    bool operator==(const ImmutableSet &rhs) const { return elts == rhs.elts; }
    bool operator!=(const ImmutableSet &rhs) const { return elts != rhs.elts; }
    bool operator< (const ImmutableSet &rhs) const { return elts <  rhs.elts; }
