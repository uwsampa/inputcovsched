    ...
    if (QueryCommand *QC = dyn_cast<QueryCommand>(D)) {
      ...
      if (QC->Values.empty() && QC->Objects.empty()) {
        ...
        if (S->getInitialValues(origQuery, QC->Objects, result)) {
          ...
          Assignment::SolutionListTy out;
          result.buildSolutionList(&out);

          for (unsigned i = 0; i < out.size(); ++i) {
            std::cout << "\tArray " << i << ":\t"
                       << out[i].first
                       << "[";
            for (unsigned j = 0; j < out[i].second.size(); ++j) {
              std::cout << (unsigned) out[i].second[j];
              if (j + 1 != out[i].second.size())
                std::cout << ", ";
            }
            std::cout << "]";
            if (i + 1 != out.size())
...
int main(int argc, char **argv) {
  ...
  // Need this so the Expr::True, etc., constants are created
