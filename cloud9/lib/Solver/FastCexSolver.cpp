...
protected:
  ...
  bool hasInitialValue(const Array& array, unsigned index) {
    assert(0 && "FIXME: check this");
    return objects.count(&array);
  }
...
public:
  ...
  CexObjectData &getObjectData(const Array *A) {
    ...
    if (!Entry)
      Entry = new CexObjectData(1/*FIXME: A->size*/);
  ...
  void propogatePossibleValues(ref<Expr> e, CexValueData range) {
    ...
    case Expr::Implies:
      return propogatePossibleValues(cast<ImpliesExpr>(e)->simplify(), range);
  ...
  void dump() {
    ...
    for (std::map<const Array*, CexObjectData*>::iterator 
      ...
      std::cerr << A->name << "\n";
      std::cerr << "possible: [";
      for (unsigned i = 0; 0/*FIXME: i < A->size*/; ++i) {
        if (i)
          std::cerr << ", ";
...
Solver *klee::createFastCexSolver(Solver *s) {
  assert(0 && "FastCexSolver is currently broken (see FIXMEs in FastCexSolver.cpp");
  return NULL; // FIXME: new Solver(new StagedSolverImpl(new FastCexSolver(), s));
}
