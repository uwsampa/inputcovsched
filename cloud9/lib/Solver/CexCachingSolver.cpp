...
bool CexCachingSolver::getAssignment(const Query& query, Assignment *&result) {
  ...
  Assignment tmp;
  bool hasSolution;
  if (!solver->impl->computeInitialValues(query, objects, tmp, hasSolution))
    return false;

  Assignment *binding;
  if (hasSolution) {
    binding = new Assignment(tmp);
    bool isold = false;
    ...
    if (DebugCexCacheCheckBinding) {
      if (!binding->satisfies(key.begin(), key.end())) {
        ...
        for (KeyType::const_iterator it = key.begin(); it != key.end(); ++it) {
          if (!binding->evaluate(*it)->isTrue()) {
            bad = *it;
            break;
          }
        }
        std::cerr << "============ BAD BINDINGS ============\n"
                  << "ismemo: " << isold << "\n"
                  << "objects: " << objects.size() << "\n"
                  << "queryeval: " << binding->evaluate(query.expr) << "\n"
                  << "=== BINDINGS ===\n";
        binding->dump(std::cerr);
        std::cerr << "=== BAD CONSTRAINT ===\n"
                  << bad << "\n";
        if (bad.get()) {
          std::cerr << "=== BAD CONSTRAINT :: READS ===\n";
          Expr::CachedReadsTy reads;
          findReads(bad, true, reads);
          for (size_t k = 0; k < reads.size(); ++k) {
            std::cerr << binding->evaluate(reads[k]) << " = " << reads[k] << "\n";
          }
        }
        std::cerr << "=== ALL CONSTRAINTS ===\n"
        ...
        tmp.clear();
        solver->impl->computeInitialValues(query, objects, tmp, hasSolution);
        assert(0 && "CexCachingsolver: invalid binding!");
      }
    }
  } else {
    binding = NULL;
...
bool CexCachingSolver::computeValidity(const Query& query,
  ...
  if (!a) {
    ...
    std::cerr << "=== QUERY ===\n"
              ...
              << query.constraints.getSummaryExpr() << "\n"
              << "=== LookupAssignment ===\n"
              << "retval: " << lookupAssignment(query, key, a) << "\n"
              << "assign: " << a << "\n";
    assert(0 && "computeValidity() must have assignment");
  }
  ...
  if (!isa<ConstantExpr>(q)) {
    ...
    std::cerr << "=== QUERY ===\n"
              ...
              << "=== ALL CONSTRAINTS ===\n"
              << query.constraints.getSummaryExpr() << "\n";
    assert(0 && "assignment evaluation did not result in constant");
  }
...
bool CexCachingSolver::computeValue(const Query& query,
  ...
  if (!a) {
    ...
    std::cerr << "=== QUERY ===\n"
              ...
              << "retval: " << lookupAssignment(query, key, a) << "\n"
              << "assign: " << a << "\n";
    assert(0 && "computeValue() must have assignment");
  }
