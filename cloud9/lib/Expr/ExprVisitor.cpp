...
ref<Expr> ExprVisitor::visit(const ref<Expr> &e) {
  if (!e.get()) {
    return NULL;
  } else if (!UseVisitorHash || isa<ConstantExpr>(e)) {
...
ref<Expr> ExprVisitor::visitActual(const ref<Expr> &e) {
  if (isa<ConstantExpr>(e)) {
    ...
    case Expr::QVar: res = visitQVar(static_cast<QVarExpr&>(ep)); break;
...
ExprVisitor::Action ExprVisitor::visitQVar(const QVarExpr&) {
  return Action::skipChildren();
}
...
ExprVisitor::Action ExprReplaceVisitor::visitExpr(const Expr &e) {
  if (e == *src.get()) {
...
ExprVisitor::Action ExprReplaceVisitor::visitExprPost(const Expr &e) {
  if (!visitPost)
    return Action::skipChildren();
  if (e == *src.get()) {
    return Action::changeTo(dst);
...
ExprVisitor::Action ExprReplaceAllVisitor::visitExpr(const Expr &e) {
  MapTy::iterator it = replacements.find(ref<Expr>(const_cast<Expr*>(&e)));
...
ExprVisitor::Action ExprSearchVisitor::visitExpr(const Expr &e) {
  if (found) {
    return Action::skipChildren();
  }
...
bool ExprSearchVisitor::findIn(const ref<Expr> &src) {
  found = false;
  visitFresh(src);
  return found;
}
