...
namespace {
  ...
  class ParserImpl : public Parser {
    ...
    ExprResult ParseAnyReadParenExpr(const Token &Name,
                                     unsigned Kind,
                                     Expr::Width ResTy);
    void ParseMatchedArgSequence(const Token &Name,
                                 TypeResult ExpectType,
                                 llvm::SmallVector<ExprResult,4> &Results);
...
DeclResult ParserImpl::ParseArrayDecl() {
 ...
 exit:
  ...
  const Array *Root;
  if (!Values.empty()) {
    Root = Array::createWithConstantValues(Label->Name, &Values[0], &Values[0] + Values.size());
  } else {
    Root = Array::createSymbolic(Label->Name);
  }
  ArrayDecl *AD = new ArrayDecl(Label, DomainType.get(), RangeType.get(), Root);
...
#define SetOK(kind, isfixed, numargs) (Kind=kind, IsFixed=isfixed,\
  ...
  case 7:
    ...
    if (memcmp(Tok.start, "Implies", 7) == 0)
      return SetOK(Expr::Implies, true, 2);
    break;
  }
...
/// ParseMatchedArgSequence - Parse a sequence of arguments who are
...
void ParserImpl::ParseMatchedArgSequence(const Token &Name,
                                         TypeResult ExpectType,
                                         llvm::SmallVector<ExprResult,4> &Results) {
  if (Tok.kind == Token::RParen) {
    Error("unexpected end of arguments.", Name);
  ...
  if (ExpectType.isValid()) {
    ...
    for (; Tok.kind != Token::RParen; ++numParsed) {
      Results.push_back(ParseExpr(ExpectType));
    }
    if (numParsed < 2) {
      Error("unexpected end of arguments.", Name);
      ConsumeRParen();
    ...
    llvm::SmallVector<NumberOrExprResult,4> tmp;
    ExprResult example;
    ...
    for (; Tok.kind != Token::RParen; ++numParsed) {
      // No example yet
      if (!example.isValid()) {
        NumberOrExprResult curr = ParseNumberOrExpr();
        if (curr.isNumber()) {
          tmp.push_back(curr);
          continue;
        }
        ExprResult e = curr.getExpr();
        if (!e.isValid()) {
          Results.push_back(e);
          continue;
        }
        // Found an example: reparse all prior numbers
        example = e;
        for (size_t k = 0; k < tmp.size(); ++k) {
          if (tmp[k].isNumber()) {
            Results.push_back(ParseNumberToken(e.get()->getWidth(), tmp[k].getNumber()));
          } else {
      ...
      // Have an example
        ...
        Results.push_back(ParseExpr(example.get()->getWidth()));
      }
    }
...
void ParserImpl::ParseMatchedBinaryArgs(const Token &Name,
                                        TypeResult ExpectType,
                                        ExprResult &LHS, ExprResult &RHS) {
  llvm::SmallVector<ExprResult,4> SubExprs;
  ParseMatchedArgSequence(Name, ExpectType, SubExprs);

  if (SubExprs.size() != 2) {
    Error("wrong number of args for expression.", Name);
    return;
  }
...
ExprResult ParserImpl::ParseBinaryParenExpr(const Token &Name,
                                            ...
                                            Expr::Width ResTy) {
  llvm::SmallVector<ExprResult,4> SubExprs;
  ParseMatchedArgSequence(Name, IsFixed ? TypeResult(ResTy) : TypeResult(), SubExprs);

  if (SubExprs.size() < 2) {
    return Builder->Constant(0, ResTy);
  }

  for (size_t k = 0; k < SubExprs.size(); ++k) {
    if (!SubExprs[k].isValid()) {
      return Builder->Constant(0, ResTy);
    }
    if (k > 0 && (SubExprs[k].get()->getWidth() != SubExprs[0].get()->getWidth())) {
      Error("type widths do not match in binary expression", Name);
      return Builder->Constant(0, ResTy);
    }
  }

  // Allow AndExpr and OrExpr to be chained indefinately
  ...
  ref<Expr> result = SubExprs.back().get();
  for (size_t k = 1; k < SubExprs.size(); ++k) {
    ref<Expr> lhs = SubExprs[SubExprs.size() - k - 1].get();
    switch (Kind) {
    case Expr::Add: result = Builder->Add(lhs, result); break;
    case Expr::Sub: result = Builder->Sub(lhs, result); break;
    case Expr::Mul: result = Builder->Mul(lhs, result); break;
    case Expr::UDiv: result = Builder->UDiv(lhs, result); break;
    case Expr::SDiv: result = Builder->SDiv(lhs, result); break;
    case Expr::URem: result = Builder->URem(lhs, result); break;
    case Expr::SRem: result = Builder->SRem(lhs, result); break;

    case Expr::AShr: result = Builder->AShr(lhs, result); break;
    case Expr::LShr: result = Builder->LShr(lhs, result); break;
    case Expr::Shl: result = Builder->Shl(lhs, result); break;

    case Expr::And: result = Builder->And(lhs, result); break;
    case Expr::Or:  result = Builder->Or(lhs, result); break;
    case Expr::Xor: result = Builder->Xor(lhs, result); break;

    case Expr::Eq:  result = Builder->Eq(lhs, result); break;
    case Expr::Ne:  result = Builder->Ne(lhs, result); break;
    case Expr::Ult: result = Builder->Ult(lhs, result); break;
    case Expr::Ule: result = Builder->Ule(lhs, result); break;
    case Expr::Ugt: result = Builder->Ugt(lhs, result); break;
    case Expr::Uge: result = Builder->Uge(lhs, result); break;
    case Expr::Slt: result = Builder->Slt(lhs, result); break;
    case Expr::Sle: result = Builder->Sle(lhs, result); break;
    case Expr::Sgt: result = Builder->Sgt(lhs, result); break;
    case Expr::Sge: result = Builder->Sge(lhs, result); break;
    default:
...
VersionResult ParserImpl::ParseVersionSpecifier() {
  ...
  if (!Res.isValid()) {
    Res = VersionResult(true, UpdateList(Array::createSymbolic(""), NULL));
  }
...
VersionResult ParserImpl::ParseVersion() {
  ...
  for (std::vector<WriteInfo>::reverse_iterator it = Writes.rbegin(), 
    ...
    if (LHS.isValid() && RHS.isValid())
      Base.addSingleByteUpdate(LHS.get(), RHS.get());
  }
...
void ArrayDecl::dump() {
  ...
  case Array::Symbolic:
    std::cout << " [] : ";
    std::cout << " : " << 'w' << Domain << " -> " << 'w' << Range << " = ";
    std::cout << " = symbolic";
    break;
  case Array::ConstantInitialValues:
    std::cout << " [ " << Root->getConstantInitialValues().size() << " ] : ";
    std::cout << " : " << 'w' << Domain << " -> " << 'w' << Range << " = ";
    std::cout << " = [";
    for (size_t i = 0; i < Root->getConstantInitialValues().size(); ++i) {
      if (i)
        std::cout << " ";
      std::cout << Root->getConstantInitialValues()[i];
    }
    std::cout << "]";
    break;
