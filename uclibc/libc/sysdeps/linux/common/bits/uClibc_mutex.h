...
// KLEE: extra declarations (see definitions in libc/misc/internals/__uClibc_main.c)
extern int __uClibc_wrap_pthread_mutex_lock(pthread_mutex_t *m) __THROW;
extern int __uClibc_wrap_pthread_mutex_trylock(pthread_mutex_t *m) __THROW;
extern int __uClibc_wrap_pthread_mutex_unlock(pthread_mutex_t *m) __THROW;
...
#define __UCLIBC_MUTEX_LOCK_CANCEL_UNSAFE(M)								\
		__uClibc_wrap_pthread_mutex_lock(&(M))

#define __UCLIBC_MUTEX_UNLOCK_CANCEL_UNSAFE(M)								\
		__uClibc_wrap_pthread_mutex_unlock(&(M))

#define __UCLIBC_MUTEX_TRYLOCK_CANCEL_UNSAFE(M)								\
		__uClibc_wrap_pthread_mutex_trylock(&(M))
	...
	do {												\
		...
		if (__infunc_need_locking) {								\
			_pthread_cleanup_push_defer(&__infunc_pthread_cleanup_buffer,			\
					   (void (*) (void *))__uClibc_wrap_pthread_mutex_unlock,	\
										&(M));			\
			__uClibc_wrap_pthread_mutex_lock(&(M));						\
		}											\
		((void)0)
