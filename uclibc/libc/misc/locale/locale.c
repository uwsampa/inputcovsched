...
int attribute_hidden _locale_set_l(const unsigned char *p, __locale_t base)
{
  ...
  // XXX: KLEE: there is an OOB acces in here: I think there's a bug in this file,
  // XXX: so I'm just commenting it all out
  klee_warning("SKIPPING _locale_set_l in uclibc/libc/misc/locale/locale.c");
  return 1;
