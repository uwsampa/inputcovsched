...
// KLEE: Disabled since we never link this without linking libpthread.a,
// and this confuses the LLVM linker
#if 0
#ifdef __UCLIBC_HAS_THREADS__
/*
...
static __always_inline int not_null_ptr(const void *p)
{
	...
	return q != 0;
#else
	return (*(const volatile void**)(&p)) != 0;
#endif
}
...
# ifndef SHARED
// XXX: KLEE: Don't have support for init/fini yet
#if 0
...
extern void (*__init_array_start []) (void) attribute_hidden;
extern void (*__fini_array_start []) (void) attribute_hidden;
#define __preinit_array_end __preinit_array_start
#define __init_array_end __init_array_start
#define __fini_array_end __fini_array_start
# endif
#endif
...
// KLEE: dummy function to force imports needed by klee's posix runtime
// Note: these calls are all completely bogus
extern int* __errno_location(void);
__attribute__((used)) void __klee_force_linkage_for_posix(void);
void __klee_force_linkage_for_posix(void)
{
  char buf[8];
  strtol(buf, NULL, 0);
  fcntl(0, 0, NULL);
