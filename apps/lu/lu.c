...

{
  ...
  KLEE_READ_VARIABLE_INPUT(n);
  KLEE_READ_VARIABLE_INPUT(block_size);
  KLEE_READ_VARIABLE_INPUT(doprint);
  KLEE_READ_VARIABLE_INPUT(dostats);
  KLEE_READ_VARIABLE_INPUT(test_result);

  // TODO: take out these constraints since lu doesn't do any sanity-checking
  // of command-line inputs?
  klee_assume(1 <= n);
  klee_assume(1 <= block_size);
  klee_assume(block_size <= n);
  ...
  for (i=0;i<P;i++) {
    last_malloc[i] = (double *) G_MALLOC_PAGE_ALIGN(proc_bytes[i]);
...

{
  ...
  klee_print_expr_only("N:", n);
  klee_print_expr_only("BS: ", block_size);
...
/* POSSIBLE ENHANCEMENT:  Here is where one might reset the
  ...
  BARRIER(Global->start, P);
  klee_print_expr_concretizations("MyNum:", MyNum, 3);
...

{
  ...
  for (k=0, K=0; k<n; k+=bs, K++) {
    ...
    //klee_print_expr_concretizations("MyNum", MyNum, 3);
    //klee_print_expr_concretizations("N", n, 3);
    //klee_print_expr_concretizations("BS", bs, 3);
#endif
...
#if defined(ENABLE_KLEE_EXPERIMENT) && ENABLE_KLEE_EXPERIMENT==2
    klee_ics_region_marker();
    //klee_print_expr_concretizations("MyNum", MyNum, 3);
#endif
  }
