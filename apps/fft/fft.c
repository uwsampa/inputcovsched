...

{
  ...
  KLEE_READ_VARIABLE_INPUT(M);
  KLEE_READ_VARIABLE_INPUT(num_cache_lines);
  KLEE_READ_VARIABLE_INPUT(log2_line_size);
  KLEE_READ_VARIABLE_INPUT(doprint);
  KLEE_READ_VARIABLE_INPUT(dostats);
  KLEE_READ_VARIABLE_INPUT(test_result);
  orig_num_lines = num_cache_lines;

  klee_assume(M%2 == 0);
  klee_assume(0 <= M);
  klee_assume(M <= 26);  // since we will alloc O(2^M) bytes
  klee_assume(1 <= num_cache_lines);
  klee_assume(0 <= log2_line_size);

  // FIXME: must be a constant for now, but we should fork for each of {2,4,8}
  // Note that we can't just say P==2||P==4||P==8||... since P is
  // passed to sqrt, concretizing it -- we have to explicitly fork.
  P = 2;

  klee_ics_begin();
#endif

  MAIN_INITENV(,80000000,P);
  ...
  // XXX: This avoids a bug in either KLEE or STP (not sure which)
  klee_assume(num_cache_lines > 0);
  ...
  x = (double *) G_MALLOC_PAGE_ALIGN(2*(N+rootN*pad_length)*sizeof(double));
  trans = (double *) G_MALLOC_PAGE_ALIGN(2*(N+rootN*pad_length)*sizeof(double));
  umain = (double *) G_MALLOC(2*rootN*sizeof(double));  
  umain2 = (double *) G_MALLOC_PAGE_ALIGN(2*(N+rootN*pad_length)*sizeof(double));
...
/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
  ...
  // XXX: Hint for STP: this avoids a constraint that STP has lots of trouble solving
  klee_assume(MyNum <= P);

  MyFirst = rootN*MyNum/P;
  MyLast = rootN*(MyNum+1)/P;
  //klee_print_range("BAR#2 rootN: ", rootN);
  //klee_print_range("BAR#2 MyNum: ", MyNum);
  //klee_print_range("BAR#2 P: ", P);
  //klee_print_range("BAR#2 MyFirst: ", MyFirst);
  //klee_print_range("BAR#2 MyLast: ", MyLast);
...

{
  ...
  BARRIER(Global->start, P);
  klee_debug("BAR#3 GO\n");
  //klee_print_expr_concretizations("BAR#3 rootN: ", rootN, 3);
  //klee_print_expr_concretizations("BAR#3 MyNum: ", MyNum, 3);
  ...
  //klee_print_expr_concretizations("BAR#3 MyFirst: ", MyFirst, 3);
  //klee_print_expr_concretizations("BAR#3 MyLast: ", MyLast, 3);
  for (j=MyFirst; j<MyLast; j++) {
    klee_debug("BAR#3 FFT1DOnce\n");
    FFT1DOnce(direction, m1, n1, upriv, &scratch[2*j*(n1+pad_length)]);
    klee_debug("BAR#3 Twiddle\n");
...

{
  ...
  //klee_print_expr_concretizations("BAR#3 blksize: ", blksize, 3);
  //klee_print_range("BAR#3 rootN: ", rootN);
  //klee_print_range("BAR#3 blksize: ", blksize);
  //klee_print_expr_concretizations("BAR#3 blksize-zero: ", blksize==0, 3);
  //klee_print_expr_concretizations("BAR#3 blksize-neg: ", blksize<0, 3);
  //klee_print_expr_concretizations("BAR#3 blksize2-neg: ", (2*blksize)<0, 3);
  //klee_print_expr_concretizations("BAR#3 blksize2-zero: ", (2*blksize)==0, 3);
  numblks = (2*blksize)/num_cache_lines;
  if (numblks * num_cache_lines != 2 * blksize) {
    numblks ++;
  }
  //klee_print_expr_concretizations("BAR#3 numblks: ", numblks, 3);
  //klee_print_expr_concretizations("BAR#3 numblks-0: ", numblks==0, 3);
  //klee_print_range("BAR#3 numblks: ", numblks);
  blksize = blksize / numblks;
  //klee_print_expr_concretizations("BAR#3 blksize-final: ", blksize, 3);
  //klee_print_range("BAR#3 blksize: ", blksize);
  firstfirst = MyFirst;
  row_count = n1/P;
  n1p = n1+pad_length;
