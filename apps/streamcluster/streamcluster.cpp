	       ...
	       int pid, pthread_barrier_t* barrier )
{
  ...
  while(1) {
    ...
    /* if number of centers seems good, try a more accurate FL */
    // KLEE: Converted floating point to integers
    //if (((k <= (1.1)*kmax)&&(k >= (0.9)*kmin))||
    //    ((k <= kmax+2)&&(k >= kmin-2)))
    if (((10*k <= 11*kmax)&&(10*k >= 9*kmin))||
        ((k <= kmax+2)&&(k >= kmin-2))) {
...
public:
  ...
  // KLEE: Dumb: had to rename these to get this to compile
...
public:
  ...
  size_t read( float* dest, int dim, int num ) {
    return fread(dest, sizeof(float)*dim, num, fp); 
  }
  int error() {
    return ferror(fp);
  }
		    ...
		    long chunksize, long centersize, char* outfile )
{
  ...
  while(1) {
    ...
    if( stream->error() || numRead < (unsigned int)chunksize && !stream->eof() ) {
...
#ifdef ENABLE_KLEE
  ////////////////////////////////////////////////////////////////
  // Klee: make data symbolic

  kmin = klee_range32(1, INT_MAX, "kmin");
  kmax = klee_range32(kmin, INT_MAX, "kmax");
  dim = 3;
  n = 0;
  chunksize = klee_range32(1, INT_MAX, "chunksize");
  clustersize = klee_range32(1, INT_MAX, "clustersize");
  // infilename
  // outfilename
  nproc = 2;
...
#else
  ////////////////////////////////////////////////////////////////
  // Standard: load data from command line
  if (argc<10) {
