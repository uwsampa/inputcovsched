    ...
    while (1) {
      ...
      if (fetch_start < fetch_count) {
          ...
          if (item.start + item.len - anchor > 0) {
            ...
            chcount ++;
            chunks_per_anchor[item.anchorid] = chcount;
            // XXX: we had this disabled for klee?
...
SendBlock(void * targs) 
{
  ...
  fd = open(conf->outfile, O_CREAT|O_TRUNC|O_WRONLY|O_TRUNC,00644);
  if (fd < 0) {
...
Encode(config * conf)
{
  ...
  */
  ...
  // TODO: KLEE: re-enable this after we get inputs working
...
#endif // ENABLE_KLEE_PTHREAD_JOINALL
