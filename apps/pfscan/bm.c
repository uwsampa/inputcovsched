          ...
          void *misc)
{
    ...
    klee_debug("BM_SEARCH START\n");
    printf("BM_SEARCH START: n=%lu y='%.20s...'\n", n, y);

    /* Searching */
    j = 0;
    while (j <= n - bmp->saved_m)
    {
        klee_debug("BM_SEARCH LOOP\n");

#ifdef ENABLE_KLEE_MANUAL_LOOP_ABSTRACTIONS
        // Here, we are "abstracting" the loop that computes "i".
        // We abstract this loop by simply assigning "i" a fresh symbolic
        // value (see below for the call to klee_read_symbolic_input()).
        ...
        // Kind of ugly: the way LLVM compiles this code, the outer while loop
        // (over "j") actually contains a nested loop that runs until a match
        // is found (that is, until "i < 0").  Hence, we get the following:
        //    while (j..) {
        //      for (..) {
        //        i = readSymbolic()
        //        if (i < 0) break;
        ...
        //      mfun(..)
        //    }
        // We need to "abstract" that inner loop as well.  Our approach is to
        // simply place a region marker in the inner loop (and note that this
        // manually-placed region marker will also cover the outer loop, as it
        // must execute at least once per outer loop iteration).
        klee_ics_region_marker();
        klee_read_symbolic_input(&i, sizeof i, "bm_search.i");
#endif

        if (i < 0)
        {
            klee_debug("BM_SEARCH FOUND?\n");
            if (mfun)
            {
                ++nm;
                ...
                c = mfun(y, n, j, misc);
                if (c) {
                    klee_debug("BM_SEARCH EARLY EXIT: BAD RET FROM MFUN!\n");
                    return (c < 0 ? c : nm);
                }
                ...
                klee_debug("BM_SEARCH EARLY EXIT: NO MFUN!\n");
                return j;
            }
        }
        else
        {
            klee_debug("BM_SEARCH TRY AGAIN\n");
