...

{
  ...
  KLEE_READ_VARIABLE_INPUT(radix);
  KLEE_READ_VARIABLE_INPUT(num_keys);
  KLEE_READ_VARIABLE_INPUT(max_key);
  KLEE_READ_VARIABLE_INPUT(doprint);
  KLEE_READ_VARIABLE_INPUT(dostats);
  KLEE_READ_VARIABLE_INPUT(test_result);

  klee_assume(2 <= radix);    // if radix==1, get_max_digits does not terminate
  klee_assume(1 <= num_keys);
  klee_assume(1 <= max_key);
  ...
  klee_ics_begin();
   ...
   MAIN_INITENV(,80000000,number_of_processors)
...
#else
   // XXX
   rank_me = (int **) G_MALLOC(number_of_processors*sizeof(int*));
   if ((global == NULL) || (key[0] == NULL) || (key[1] == NULL) ||
       (key_partition == NULL) || (rank_partition == NULL) || 
       (rank_me == NULL)) {
     fprintf(stderr,"ERROR: Cannot malloc enough memory\n");
   ...
   for (i=0;i<number_of_processors;i++) {
     rank_me[i] = (int*) G_MALLOC(number_of_processors*radix*sizeof(int));
     if (rank_me[i] == NULL) {
       fprintf(stderr,"ERROR: Cannot malloc enough memory\n");
       exit(-1); 
     }
   ...
   for (i=0;i<number_of_processors;i++) {
     // XXX FIXME: was this a bugfix?
     klee_print_expr_concretizations("RADIX_AT_ALLOC", radix, 5);
     gp[i].rank_ff = (int *) G_MALLOC(48*radix*sizeof(int)+PAGE_SIZE);
...
void slave_sort()
{
   ...
   // XXX: HACK: This should be better
   MyNum = klee_get_concrete_thread_context();
   //klee_assume(0 <= MyNum);
   //klee_assume(MyNum < number_of_processors);
...
/* Do 1 iteration per digit.  */
   ...
   for (loopnum=0;loopnum<max_num_digits;loopnum++) {
     ...
     // XXX: HACK: This should be better
     MyNum = klee_get_concrete_thread_context();
     //klee_print_expr_only("MyNum: ", MyNum);
     //klee_print_expr_only("PfxTreeLow: ", &global->prefix_tree[0]);
     //klee_print_expr_only("PfxTreeLow: ", &global->prefix_tree[1]);
     //klee_print_expr_only("PfxTreeHi: ", &global->prefix_tree[number_of_processors]);
     //klee_print_expr_only("PfxTreeHi: ", &global->prefix_tree[number_of_processors+1]);
...
/* generate histograms based on one digit */
     ...
     // XXX: HACK: This should be better
     MyNum = klee_get_concrete_thread_context();
     //klee_assume((unsigned)MyNum < (unsigned)number_of_processors);
